...
const recipeVariantUpsertSql = `
  ...
  ON CONFLICT(id) DO UPDATE SET
    ...
    version = excluded.version
`;
const buildRecipeVariantUpsertParams = (rv) => ({
  id: rv.id,
...
`;
const buildComponentUpsertParams = (c) => ({
  id: c.id,
  name: c.name ?? null,
  reference: c.reference ?? null,
...
`;
const buildComponentItemUpsertParams = (ci) => ({
  id: ci.id,
...
`;
const buildComponentLotUpsertParams = (cl) => ({
  id: cl.id,
...
const componentLotLogUpsertSql = `
...
`;
const buildComponentLotLogUpsertParams = (cll) => ({
  id: cll.id,
...
class DatabaseService {
  ...
  applyPullData(payload) {
    ...
    const tx = this.transaction(() => {
      ...
      if (Array.isArray(data.recipeVariants) && data.recipeVariants.length > 0) {
      ...
      if (Array.isArray(data.componentItems) && data.componentItems.length > 0) {
      ...
      if (Array.isArray(data.componentLotLogs) && data.componentLotLogs.length > 0) {
...
class AuthService {
  ...
  async storeTokens(payload) {
    try {
      this.db.putCache(TOKENS_CACHE_KEY, payload);
    } catch (error2) {
      console.error("Failed to store tokens in cache", error2);
    }
    try {
      const serialized = JSON.stringify(payload);
      await keytar.setPassword(SERVICE_NAME, ACCOUNT_NAME, serialized);
    } catch (error2) {
      console.error("Failed to store tokens in keytar", error2);
    }
  }
  async clearTokens() {
    try {
      ...
      console.error("Failed to clear tokens from cache", error2);
    }
    try {
      await keytar.deletePassword(SERVICE_NAME, ACCOUNT_NAME);
    } catch (error2) {
      console.error("Failed to clear tokens in keytar", error2);
    }
  }
  async getTokens() {
    try {
      const cached = this.db.getCache(TOKENS_CACHE_KEY);
      const accessToken = cached?.accessToken != null ? String(cached.accessToken) : "";
      const refreshToken = cached?.refreshToken != null ? String(cached.refreshToken) : "";
      if (accessToken && refreshToken) {
        return { accessToken, refreshToken };
      }
    } catch (error2) {
      console.error("Failed to read tokens from cache", error2);
    }
    try {
      const serialized = await keytar.getPassword(SERVICE_NAME, ACCOUNT_NAME);
      if (!serialized) return null;
      const parsed = JSON.parse(serialized);
      const accessToken = parsed?.accessToken != null ? String(parsed.accessToken) : "";
      const refreshToken = parsed?.refreshToken != null ? String(parsed.refreshToken) : "";
      ...
      console.error("Failed to read tokens from keytar", error2);
      return null;
    }
  }
  getUser() {
...
class P2PService {
  ...
  startTcpServer(onMessage) {
    this.tcpServer = net$1.createServer((socket) => {
      socket.on("data", (data) => {
        try {
          const str2 = data.toString();
          const json2 = JSON.parse(str2);
          if (json2.app === APP_ID && json2.payload) {
            onMessage(json2.payload);
          }
        } catch {
        }
      });
  ...
  startUdpDiscovery() {
    ...
    this.udpSocket.on("message", (msg, rinfo) => {
      try {
        const json2 = JSON.parse(msg.toString());
        if (json2.app === APP_ID && json2.deviceId !== this.deviceId) {
          if (!this.peers.has(json2.deviceId)) {
            this.peers.set(json2.deviceId, {
              ip: rinfo.address,
              port: json2.tcpPort
            });
          }
  ...
  async sendToPeerById(deviceId, payload) {
...
class AssetService {
  constructor(p2pService2) {
    this.pendingRequests = /* @__PURE__ */ new Map();
    this.p2pService = p2pService2;
    this.initP2P();
    this.initProtocol();
  }
  getHash(url) {
  ...
  async handleAssetRequest(payload) {
...
const updateLabelSettings = async (db, payload) => {
  const { outletId, settings } = payload;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  db.run(
    `
...
const updateInvoiceSettings = async (db, payload) => {
  const { outletId, settings } = payload;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  db.run(
    `
...
function createWindow() {
  ...
  if (!fs$1.existsSync(iconPath) && app.isPackaged) {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    ...
    (_event, payload) => updateLabelSettings(dbService, payload)
  );
  ipcMain.handle(
...
`
  ...
  ipcMain.handle("net:deleteImage", async (_event, { publicId }) => {
    ...
    const timestamp2 = Math.round((/* @__PURE__ */ new Date()).getTime() / 1e3).toString();
    const signatureStr = `public_id=${pid}&timestamp=${timestamp2}${apiSecret}`;
    const signature = crypto.createHash("sha1").update(signatureStr).digest("hex");
...
`));
    chunks.push(
      Buffer.from(
        `Content-Disposition: form-data; name="public_id"\r
\r
${pid}\r
--${boundary}\r
`
...
`
    ...
    try {
      ...
      console.error("[Main] Cloudinary delete error:", error2);
      return { ok: false, status: 500, error: error2.message };
    }
  });
  ...
  ipcMain.on(
    "p2p:broadcast",
    (_event, payload) => p2pService.broadcast(payload)
  );
  ipcMain.on(
    "p2p:sendToPeer",
    (_event, deviceId2, payload) => p2pService.sendToPeerById(deviceId2, payload)
  );
