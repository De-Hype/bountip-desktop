...
BANKS_BY_COUNTRY.Nigeria = BANKS_BY_COUNTRY.NG;
...
export const getBanksByCountry = (countryCode: string): string[] => {
  ...
  if (!raw) return [];
  const direct = BANKS_BY_COUNTRY[raw];
  if (direct) return direct;
  const upper = raw.toUpperCase();
  const byUpper = BANKS_BY_COUNTRY[upper];
  if (byUpper) return byUpper;
  const nameToCode =
    Object.entries(COUNTRY_NAMES).find(
      ([, name]) => name.toLowerCase() === raw.toLowerCase(),
    )?.[0] || null;
  if (nameToCode && BANKS_BY_COUNTRY[nameToCode])
    return BANKS_BY_COUNTRY[nameToCode];
  const byKeyMatch =
    Object.entries(BANKS_BY_COUNTRY).find(
...
export const searchBanks = (
  query: string,
): Array<{ country: string; bank: string }> => {
