...
type TotalExpiredItemsModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
const resolveDisplayedUnit = (row: TotalExpiredRow) => {
...
const TotalExpiredItemsModal = ({
  isOpen,
  onClose,
  outletId,
  outletName,
  currencyCode,
}: TotalExpiredItemsModalProps) => {
  const { showToast } = useToastStore();
  const [isLoading, setIsLoading] = useState(false);
  const [isExporting, setIsExporting] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [rows, setRows] = useState<TotalExpiredRow[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchRows = async () => {
      ...
      try {
          ...
          (await api.dbQuery(
            `
              SELECT
                ...
                COALESCE(il.currentStockLevel, 0) AS quantityExpired,
              ...
              ORDER BY datetime(il.expiryDate) DESC
            `,
            [outletId, startOfToday],
          )) || [];

        const mapped: TotalExpiredRow[] = (result as any[]).map((r) => ({
          ...
          supplierSerialNumber:
            r?.supplierSerialNumber != null
              ...
              : "",
          quantityExpired: Number(r?.quantityExpired || 0),
        ...
        console.error("Failed to fetch expired items rows:", err);
        setRows([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const exportCsv = async () => {
    ...
    try {
      const data = filteredRows.map((r) => {
        ...
        const qty = Number(r.quantityExpired || 0);
      ...
      XLSX.writeFile(wb, `expired_items_${safeOutletName}_${date}.csv`, {
        bookType: "csv",
      });

      showToast("success", "Export complete", "CSV file downloaded");
    } catch (err) {
      console.error("Failed to export expired items:", err);
...
export default TotalExpiredItemsModal;
