...
type TotalLowStockItemsModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
const resolveDisplayedUnit = (row: TotalLowStockRow) => {
...
const TotalLowStockItemsModal = ({
  isOpen,
  onClose,
  outletId,
  outletName,
  currencyCode,
}: TotalLowStockItemsModalProps) => {
  const { showToast } = useToastStore();
  const [isLoading, setIsLoading] = useState(false);
  const [isExporting, setIsExporting] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [rows, setRows] = useState<TotalLowStockRow[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchRows = async () => {
      ...
      try {
          ...
          (await api.dbQuery(
            `
              ...
              SELECT
                ...
                COALESCE(lot_min.minExpiry, '') AS expiryDate,
              ...
              WHERE i.outletId = ?
                AND ii.isDeleted = 0
                AND COALESCE(ii.minimumStockLevel, 0) > 0
                AND COALESCE(ii.currentStockLevel, 0) > 0
                AND COALESCE(ii.currentStockLevel, 0) < COALESCE(ii.minimumStockLevel, 0)
              ORDER BY im.name ASC
            `,
            [outletId],
          )) || [];

        const mapped: TotalLowStockRow[] = (result as any[]).map((r) => ({
          inventoryItemId:
        ...
        console.error("Failed to fetch low stock rows:", err);
        setRows([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const exportCsv = async () => {
    ...
    try {
      const data = filteredRows.map((r) => {
        ...
        const valueLabel = `${currencySymbol}${new Intl.NumberFormat(
          undefined,
          {
        ...
        return {
          itemCode: r.itemCode,
          itemName: r.itemName,
          currentStock: currentStockLabel,
          value: valueLabel,
      ...
      const ws = XLSX.utils.json_to_sheet(data, {
        header: ["itemCode", "itemName", "currentStock", "value", "expiryDate"],
      });
      ...
      XLSX.writeFile(wb, `low_stock_items_${safeOutletName}_${date}.csv`, {
        bookType: "csv",
      });

      showToast("success", "Export complete", "CSV file downloaded");
    } catch (err) {
      console.error("Failed to export low stock items:", err);
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {paginatedRows.map((r) => {
                        ...
                        return (
                          <tr
                            key={r.inventoryItemId || r.itemCode}
                          ...
                          >
                            ...
                            <td className="px-6 py-4">{currentStockLabel}</td>
...
export default TotalLowStockItemsModal;
