...
type TotalOutOfStockItemsModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
const resolveDisplayedUnit = (row: TotalOutOfStockRow) => {
...
const TotalOutOfStockItemsModal = ({
  isOpen,
  onClose,
  outletId,
  outletName,
  currencyCode,
}: TotalOutOfStockItemsModalProps) => {
  const { showToast } = useToastStore();
  const [isLoading, setIsLoading] = useState(false);
  const [isExporting, setIsExporting] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [rows, setRows] = useState<TotalOutOfStockRow[]>([]);
      ...
      try {
        const result =
          (await api.dbQuery(
            ...
            `,
            [outletId, startOfToday],
          )) || [];

        const mapped: TotalOutOfStockRow[] = (result as any[]).map((r) => ({
        ...
        console.error("Failed to fetch out of stock rows:", err);
        setRows([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return itemName.includes(q) || itemCode.includes(q) || lotNumber.includes(q);
    });
  }, [rows, searchQuery]);
    ...
    try {
      const data = filteredRows.map((r) => {
        ...
        return {
          ...
          value: `${currencySymbol}${new Intl.NumberFormat(undefined, {
      ...
      XLSX.writeFile(wb, `out_of_stock_items_${safeOutletName}_${date}.csv`, {
        bookType: "csv",
      });

      showToast("success", "Export complete", "CSV file downloaded");
    } catch (err) {
      console.error("Failed to export out of stock items:", err);
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              {r.expiryDate ? formatShortDate(r.expiryDate) : "-"}
                            </td>
                            <td className="px-6 py-4">
                              {days != null ? days : "-"}
                            </td>
...
export default TotalOutOfStockItemsModal;
