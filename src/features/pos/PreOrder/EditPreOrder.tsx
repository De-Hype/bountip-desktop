...
const EditPreOrder = ({
  ...
  const canProcessPayment = paymentLabel === "Unpaid";
  ...
  const saveEdits = useCallback(
    async (next: {
      ...
      try {
        ...
        if (next.paymentStatus === "Verified") {
          timelineArr.push({
        ...
        await api.dbQuery(
          ...
          [
            ...
            cleanedItems.length,
            cleanedItems.reduce(
              (acc, it) => acc + (Number(it.quantity || 0) || 0),
              0,
            ),
            subtotalAmount,
            customerId || null,
        ...
        setOrder((prev: any) => ({
          ...
          cancelledAt: next.cancelledAt ?? prev?.cancelledAt,
        ...
        showToast("error", "Update failed", "Failed to update order");
        return false;
      }
    },
    [
                ...
                {activeStep === "additional" && (
                  <>
                    ...
                    <div className="border-t border-dashed border-[#9CA3AF] pt-10">
                      ...
                      <div className="mt-6 text-[13px] text-[#6B7280]">
                        Payment Status:{" "}
              ...
              <button
                ...
                onClick={async () => {
                  const ok = await saveEdits({
                    status: "Confirmed",
                    paymentStatus:
                      String(order?.paymentStatus ?? "").trim() || "Pending",
