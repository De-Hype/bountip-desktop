...
//   const [shouldRefreshTotals, setShouldRefreshTotals] = useState(true);
...
//     return Math.max(1, Math.ceil((totalItems || 0) / (limit || 10)));
//   }, [limit, totalItems]);
...
//   const fetchTotals = async (serverTotal: number) => {
//     if (!selectedOutlet?.id) return;
//     const total = Number(serverTotal || 0) || 0;
//     if (!total) {
//       setTotalItems(0);
//       setTotalAvailable(0);
...
//         (p) => p.availableAtStorefront,
//       ).length;
//       setTotalItems(total);
//       setTotalAvailable(availableCount);
//       setTotalUnavailable(products.length - availableCount);
//       return;
//     }
...
//       (product) => product.availableAtStorefront,
//     ).length;

//     setTotalItems(productList.length);
//     setTotalAvailable(availableCount);
//     setTotalUnavailable(productList.length - availableCount);
//   };
...
//       const serverTotal = Number(meta?.total || 0) || 0;
//       if (shouldRefreshTotals) {
//         await fetchTotals(serverTotal);
//         setShouldRefreshTotals(false);
//       } else if (!totalItems) {
...
//   useEffect(() => {
//     setPage(1);
//     setShouldRefreshTotals(true);
//   }, [selectedOutlet?.id]);

//   useEffect(() => {
//     fetchProducts();
//   }, [selectedOutlet?.id, page, limit]);
...
//         {isLoadingProducts ? (
//           <div className="py-10 text-center text-sm text-[#737373] italic">
//             Loading products...
//           </div>
//         ) : (
//           products.map((product) => {
//             const isAvailable = product.availableAtStorefront;
...
//                 <div className="flex items-center gap-4">
//                   <img
//                     src={product.logoUrl || AssetsFiles.product}
//                     alt={product.name}
