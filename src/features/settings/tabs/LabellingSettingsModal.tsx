...
export const LabellingSettingsModal: React.FC<LabellingSettingsModalProps> = ({
  ...
  useEffect(() => {
    ...
    const outletWithLogo = outlet as unknown as {
      logoUrl?: string | null;
    } | null;
    setImageUrl(settings.customizedLogoUrl ?? outletWithLogo?.logoUrl ?? null);
    setLogoError(null);
  }, [isOpen, outlet]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const labelSettingsDto: LabelSettingsDto = {
      ...
      showManufacturingDate:
        formData.labelItems.find((item) => item.key === "showManufacturingDate")
          ?.enabled ?? true,
      ...
      showAllergenInfo:
        formData.labelItems.find((item) => item.key === "showAllergenInfo")
          ?.enabled ?? true,
      showPrice:
        formData.labelItems.find((item) => item.key === "showPrice")?.enabled ??
        true,
    };
