...
export const ReceiptSettingsModal: React.FC<ReceiptSettingsModalProps> = ({
  ...
  useEffect(() => {
    ...
    setImageUrl(outletWithLogo?.logoUrl ?? null);
  }, [isOpen, outlet]);
  ...
  const mapFormDataToDto = (): ReceiptSettingsDto => {
    const getFontStyle = (
      fontStyle: string,
    ): ReceiptSettingsDto["fontStyle"] => {
      const validFonts: ReceiptSettingsDto["fontStyle"][] = [
        "Times New Roman",
        "Arial",
        "Helvetica",
        "Courier",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!outlet?.id) {
      ...
      showToast("error", "Missing Outlet ID", message);
      return;
    }
